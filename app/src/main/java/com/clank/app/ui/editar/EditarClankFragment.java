...
public class EditarClankFragment extends Fragment {
  ...
  private void intentarVolver(View v) {
    if (viewModel.hayCambios()) {
      mostrarConfirmarDescartarCambios(v);
    } else {
  ...
  private void configurarListeners(View v) {
    ...
    binding.btnGuardarBoceto.setOnClickListener(b -> guardarBoceto(v));
  ...
  private void actualizarEstadoBotonPublicar() {
    boolean valido = !binding.etTitulo.getText().toString().trim().isEmpty()
  ...
  /////////////////////////imagen (opcional: cambiar por estilo clank)/////////////////////////
  ...
  private void mostrarDialogoAccionesImagen(Object img, View fila) {
    new AlertDialog.Builder(requireContext())
      .setItems(new CharSequence[]{
        ...
        if (which == 0) {
          Uri uri = img instanceof Uri ? (Uri) img : Uri.parse((String) img);
  ...
  private void anyadirFilaInstruccion() {
    ...
    fila.findViewById(R.id.llBotonImagenInstruccion)
      .setOnClickListener(b -> mostrarDialogoSeleccionImagen(fila));

    fila.findViewById(R.id.ivPreviewInstruccion).setOnClickListener(b -> {
      Object tag = fila.getTag(R.id.ivPreviewInstruccion);
      if (tag != null) mostrarDialogoAccionesImagen(tag, fila);
    });
  ...
  private void intentarPublicar(View v) {
    ...
    viewModel.publicarClank(
      titulo, descripcion, tiempoSeleccionado,
      uriPortadaNueva, urlPortadaActual,
      recogerMateriales(), recogerHerramientas(),
      recogerTextosInstrucciones(), recogerImagenesInstrucciones(),
  ...
  private void mostrarConfirmarDescartarCambios(View v) {
    new AlertDialog.Builder(requireContext())
      .setTitle(getString(R.string.editar_descartar_cambios_titulo))
      .setMessage(getString(R.string.editar_descartar_cambios_mensaje))
      .setPositiveButton(getString(R.string.editar_descartar_confirmar), (d, w) ->
        Navigation.findNavController(v).navigateUp())
      .setNegativeButton(getString(R.string.cancelar), null)
      .show();
  }
    ...
    viewModel.getEstadoGuardar().observe(getViewLifecycleOwner(), recurso -> {
      ...
      switch (recurso.estado) {
        ...
        case EXITO:
          ...
          Bundle args = new Bundle();
          args.putString("tabInicial", esPublicacion ? "clanks" : "bocetos");
  ...
  private void rellenarFormulario(EditarClankViewModel.DatosClank datos) {
    ...
    if (datos.tiempo >= 0) seleccionarTiempo(datos.tiempo);

    if (!datos.portadaUrl.isEmpty()) {
      binding.ivPortadaPreview.setVisibility(View.VISIBLE);
      binding.llAnyadirPortada.setVisibility(View.GONE);
      Glide.with(this).load(datos.portadaUrl).centerCrop()
    ...
    if (datos.instrucciones.isEmpty()) {
      ...
      for (String[] instruccion : datos.instrucciones)
        anyadirFilaInstruccionConDatos(instruccion[0], instruccion[1]);
    }
  }
  private void anyadirFilaMaterialConDatos(String cantidad, String nombre) {
    View fila = LayoutInflater.from(requireContext())
      .inflate(R.layout.item_material, binding.llContenedorMateriales, false);
  ...
  private void anyadirFilaHerramientaConDatos(String nombre) {
    View fila = LayoutInflater.from(requireContext())
      .inflate(R.layout.item_material, binding.llContenedorHerramientas, false);
    fila.findViewById(R.id.etCantidad).setVisibility(View.GONE);
    ((EditText) fila.findViewById(R.id.etNombreElemento)).setText(nombre);
    ((EditText) fila.findViewById(R.id.etNombreElemento))
      .setHint(getString(R.string.crear_hint_herramienta));
  ...
  private void anyadirFilaInstruccionConDatos(String texto, @Nullable String urlImagen) {
    View fila = LayoutInflater.from(requireContext())
      .inflate(R.layout.item_instruccion, binding.llContenedorInstrucciones, false);
    ...
    if (urlImagen != null && !urlImagen.isEmpty()) {
      ...
      boton.setVisibility(View.GONE);
      preview.setVisibility(View.VISIBLE);
      Glide.with(this).load(urlImagen).centerCrop().into(preview);
    }
  ...
  private void cargarChipsCategorias(List<String[]> categorias) {
    ...
    List<String> seleccionadas = datos != null ? datos.categorias : new ArrayList<>();
    ...
    for (String[] cat : categorias) {
      ...
      chip.setBackgroundResource(activo
        ? R.drawable.bg_boton_principal
        : R.drawable.bg_boton_secundario);
      chip.setTextColor(ContextCompat.getColor(requireContext(), activo
        ? R.color.clank_background_light
        : R.color.color_texto_inactivo));
      ...
      chip.setOnClickListener(b -> {
        ...
        chip.setBackgroundResource(!estaActivo
          ? R.drawable.bg_boton_principal
          : R.drawable.bg_boton_secundario);
        chip.setTextColor(ContextCompat.getColor(requireContext(), !estaActivo
          ? R.color.clank_background_light
          : R.color.color_texto_inactivo));
        viewModel.marcarCambios();
        actualizarEstadoBotonPublicar();
