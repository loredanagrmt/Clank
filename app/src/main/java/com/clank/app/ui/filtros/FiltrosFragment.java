...
import com.clank.app.ui.filtros.FiltrosViewModel;
...
public class FiltrosFragment extends Fragment {
    ...
    public void onResume() {
        super.onResume();
        ((NavbarHost) requireActivity()).mostrarNavbarConVolver(getString(R.string.filtrar_titulo));
    }
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        viewModel.categorias.observe(getViewLifecycleOwner(), categorias -> {
            ...
            for (Categoria cat : categorias) {
                Button chip = (Button) LayoutInflater.from(requireContext()).inflate(R.layout.bt_secundario, binding.chipGroupCategorias, false);

                chip.setText(cat.getCategoria());
                chip.setTag(cat.getCatId());

                ViewGroup.MarginLayoutParams lp = (ViewGroup.MarginLayoutParams) chip.getLayoutParams();
                lp.setMargins(0, 0, (int) getResources().getDimension(R.dimen.chip_margin), (int) getResources().getDimension(R.dimen.chip_margin));
