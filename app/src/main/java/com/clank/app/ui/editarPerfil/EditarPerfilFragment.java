...
public class EditarPerfilFragment extends Fragment {
  ...
  @Override
  public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup contenedor, Bundle estadoGuardado) {
  ...
  public void onSaveInstanceState(@NonNull Bundle estadoSalida) {
    ...
    if (uriFotoPerfil != null) estadoSalida.putString(CLAVE_URI_FOTO_PERFIL, uriFotoPerfil.toString());
    if (uriFotoCamaraTemporal != null) estadoSalida.putString(CLAVE_URI_FOTO_CAMARA_TEMPORAL, uriFotoCamaraTemporal.toString());
  }
  ...
  private void configurarNavbar() {
    binding.navbar.tvNavbarTitulo.setText(getString(R.string.editar_perfil_titulo));
  ...
  private void configurarFormulario() {
    // Nombre
    binding.inputNombre.tvInputTitulo.setText(getString(R.string.nombre_completo));
    binding.inputNombre.customEditText.setHint(getString(R.string.perfil_hint_nombre));
    ...
    //guardar
    binding.btnGuardar.btnPrincipal.setText(getString(R.string.editar_perfil_guardar));

    binding.tvCambiarContrasenya.setPaintFlags(
      binding.tvCambiarContrasenya.getPaintFlags() | android.graphics.Paint.UNDERLINE_TEXT_FLAG);
  }

  private void configurarPopup() {
    binding.capaPopup.setVisibility(View.GONE);
    binding.popup.tvTituloPopup.setText(getString(R.string.popup_editar_perfil_titulo));
    binding.popup.tvMensajePopup.setText(getString(R.string.popup_editar_perfil_mensaje));
    ...
    binding.popup.btnPopupAccion.btnSecundario.setText(getString(R.string.continuar));
  }
  ...
  private void seleccionarFotoPerfil() {
    ...
    new AlertDialog.Builder(requireContext())
      ...
      .setItems(opciones, (dialogo, posicion) -> {
        if (posicion == 0) launcherGaleria.launch("image/*");
  ...
  private void restaurarPreviewFotoPerfil() {
    if (uriFotoPerfil != null) mostrarPreviewFotoPerfil(uriFotoPerfil);
  }
  ...
  private void procesarGuardarCambios() {
    ...
    if (nombre.isEmpty()) {
      mostrarError(binding.inputNombre.customEditText, R.string.registro_error_nombre_vacio);
      return;
    }
    if (usuarioClank.isEmpty()) {
      mostrarError(binding.inputUsuarioClank.customEditText,
        R.string.completar_perfil_error_usuario_clank_vacio);
      return;
    }

    viewModel.guardarCambios(nombre, usuarioClank, telefono, uriFotoPerfil);
  }
  ...
  private void observarViewModel() {
    viewModel.getPerfil().observe(getViewLifecycleOwner(), usuario -> {
      ...
      binding.inputNombre.customEditText.setText(usuario.getNombre());
      binding.inputUsuarioClank.customEditText.setText(usuario.getUsuarioClank());
      binding.inputTelefono.customEditText.setText(usuario.getTelefono());
      binding.inputCorreo.customEditText.setText(usuario.getCorreo());
      binding.inputFechaNacimiento.customEditText.setText(usuario.getFechaNacimiento());
  ...
  private void gestionarError(String error) {
    ...
    if (EditarPerfilViewModel.ERROR_SUBIR_FOTO.equals(error)) {
      mostrarError(binding.inputNombre.customEditText,
    ...
    mostrarError(binding.inputNombre.customEditText, R.string.error_generico);
  }
  ...
  private void bloquearFormulario(boolean bloqueado) {
    ...
    binding.btnGuardar.btnPrincipal.setText(bloqueado
